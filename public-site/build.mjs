...
try {
  await bundle({
    ...
    outfile: fileURLToPath(renderer),
    bundle: true,
    platform: 'node',
    format: 'esm',
    packages: 'external',
  ...
  const template = await readFile(
    new URL('dist-public/index.html', root),
    'utf8',
  );
  await writeFile(
    new URL('dist-public/index.html', root),
  ...
  await writeFile(
    new URL('dist-public/demo/index.html', root),
    template
      .replace('<div id="root"></div>', `<div id="root">${render(true)}</div>`)
      .replace(
        'href="https://vergecommon.com/"',
        'href="https://vergecommon.com/demo/"',
      ),
  );
  // The public preview has no service worker or installable authenticated app.
  for (const name of ['sw.js', 'manifest.webmanifest'])
    await rm(new URL(`dist-public/${name}`, root), { force: true });
} finally {
