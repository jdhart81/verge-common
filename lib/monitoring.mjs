import { area as geodesicArea } from '@turf/area';
...
export function validateBoundary(input) {
  ...
  const geometry = { type: 'Polygon', coordinates: [ring.map((p) => [...p])] };
  const areaSquareMetres = geodesicArea(geometry);
  if (!Number.isFinite(areaSquareMetres) || areaSquareMetres <= 0)
    throw new Error('Boundary must enclose a measurable area.');
  return { geometry, bbox, areaSquareMetres, areaMethod: 'turf-geodesic-v1' };
}

// These are estimates from the submitted WGS84 boundary, never survey results.
// Turf uses a spherical Earth model: https://turfjs.org/docs/api/area
export function boundaryOverlapSquareMetres(left, right) {
  const a = validateBoundary(left),
    b = validateBoundary(right);
  if (
    a.bbox[2] <= b.bbox[0] ||
    b.bbox[2] <= a.bbox[0] ||
    a.bbox[3] <= b.bbox[1] ||
  ...
  const shared = intersect({
    ...
    features: [a.geometry, b.geometry].map((geometry) => ({
      type: 'Feature',
      properties: {},
      geometry,
    })),
  });
  return shared ? geodesicArea(shared) : 0;
}
