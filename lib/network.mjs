import { parcelSnapshot, assessmentIsCurrent } from './readiness.mjs';
  ...
  if (op === 'request_membership' || op === 'accept_invitation') {
    ...
    switch (op) {
      ...
      case 'review_partnership': {
        ...
        review(partner, user.id, choose(p.decision, ['approve', 'reject']), now);
        break;
      }
      case 'revoke_partnership': {
        requireSteward(s, user.id);
        const partner = find(s.partnerships ?? [], p.id);
        if (partner.status !== 'reviewed') fail('Only reviewed partner records can be revoked.');
      ...
      case 'review_assessment': {
        ...
        if (p.decision === 'approve' && !assessmentIsCurrent(s, assessment))
          fail('Land records changed or this legacy assessment lacks a snapshot. Record a new assessment before approval.');
        review(assessment, user.id, choose(p.decision, ['approve', 'reject']), now);
        break;
      }
