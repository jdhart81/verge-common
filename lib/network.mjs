import {
  parcelSnapshot,
  assessmentIsCurrent,
  consentLandSnapshot,
  parcelConsentIsCurrent,
  agreementCoverageSnapshot,
  agreementIsCurrent,
  parcelAgreementCovered,
  landPoolGeometry,
  pooledParcels,
} from './readiness.mjs';
import { validateAnalysis } from './analysis.mjs';
import { validateBoundary } from './monitoring.mjs';
import { currencies } from './international.mjs';
import { communityContentIssue } from './content-safety.mjs';
import { observationDateMetadata } from './observation-date.mjs';
import { applyPartnerParticipation, partnerParticipationView } from './partner-participation.mjs';
import { isWorkspaceSafetyCommand, workspaceCapacity, workspaceCapacityIssue } from './workspace-capacity.mjs';
import { allocationReconciliation } from './allocation-reconciliation.mjs';
// Shared domain rules. Every command is applied to a copy and saved atomically.
export class DomainError extends Error {
  constructor(message, status = 400) {
    super(message);
    this.status = status;
  }
}
const fail = (message, status = 400) => {
  throw new DomainError(message, status);
};
const required = (value, max = 200) => {
  if (typeof value !== 'string' || !value.trim() || value.length > max)
    fail(`Enter text between 1 and ${max} characters.`);
  return value.trim();
};
const optional = (value, max = 2000) =>
  value === undefined || value === '' ? '' : required(value, max);
const integer = (v, max = 1e12) => {
  if (!Number.isSafeInteger(v) || v < 0 || v > max)
    fail(`Enter a whole number between 0 and ${max}.`);
  return v;
};
const choose = (v, options) => {
  if (!options.includes(v)) fail('Unsupported option.');
  return v;
};
const find = (items, id) =>
  items.find((x) => x.id === id) ?? fail('Record not found.', 404);
export function membership(state, userId) {
  return state.members.find(
    (m) => m.userId === userId && m.status === 'active',
  );
}
export function isSteward(state, userId) {
  return membership(state, userId)?.role === 'steward';
}
function blockedInteraction(state, first, second) {
  return (state.blocks ?? []).some(
    (b) =>
      (b.userId === first && b.blockedUserId === second) ||
      (b.userId === second && b.blockedUserId === first),
  );
}
export function requireMember(state, userId) {
  return (
    membership(state, userId) ??
    fail('Active co-op membership is required.', 403)
  );
}
export function requireSteward(state, userId) {
  if (!isSteward(state, userId))
    fail('A co-op steward must perform this action.', 403);
}
export function newWorkspace(input, user, now, id) {
  const contentIssue = communityContentIssue('create_workspace', input);
  if (contentIssue) fail(contentIssue);
  return {
    id,
    name: required(input.name, 120),
    region: required(input.region, 120),
    summary: required(input.summary, 1000),
    visibility: 'private',
    country: optional(input.country, 120),
    currency: choose(input.currency ?? 'USD', currencies),
    ownerId: user.id,
    createdAt: now,
    updatedAt: now,
    members: [
      {
        id: crypto.randomUUID(),
        userId: user.id,
        name: required(input.displayName, 80),
        role: 'steward',
        status: 'active',
        joinedAt: now,
      },
    ],
    organization: null,
    partnerships: [],
    invitations: [],
    assessments: [],
    authority: null,
    parcels: [],
    satelliteSearches: [],
    observations: [],
    analysisJobs: [],
    analysisResults: [],
    projects: [],
    updates: [],
    events: [],
    comments: [],
    reports: [],
    blocks: [],
    tasks: [],
    agreements: [],
    evidence: [],
    proposals: [],
    charters: [],
    lots: [],
    settlements: [],
    allocations: [],
    retirements: [],
    audit: [],
  };
}
export function allocateCents(total, members, stewardshipBps, treasuryBps) {
  integer(total);
  integer(stewardshipBps, 10000);
  integer(treasuryBps, 10000);
  if (stewardshipBps + treasuryBps > 10000)
    fail('Co-op budgets cannot exceed 100%.');
  if (
    !members.length ||
    new Set(members.map((m) => m.id)).size !== members.length
  )
    fail('Use unique member IDs.');
  if (members.reduce((s, m) => s + integer(m.shareBps, 10000), 0) !== 10000)
    fail('Member shares must total 100%.');
  const gross = BigInt(total),
    base = 10000n;
  const stewardship = Number((gross * BigInt(stewardshipBps)) / base),
    treasury = Number((gross * BigInt(treasuryBps)) / base),
    pool = total - stewardship - treasury;
  const rows = members.map((m) => ({
    id: m.id,
    name: m.name,
    cents: Number((BigInt(pool) * BigInt(m.shareBps)) / base),
    r: (BigInt(pool) * BigInt(m.shareBps)) % base,
  }));
  rows.sort((a, b) =>
    a.r === b.r ? (a.id < b.id ? -1 : 1) : a.r > b.r ? -1 : 1,
  );
  let left = pool - rows.reduce((s, m) => s + m.cents, 0);
  for (const m of rows) {
    if (left > 0) {
      m.cents++;
      left--;
    }
  }
  return {
    grossCents: total,
    stewardshipCents: stewardship,
    treasuryCents: treasury,
    memberPoolCents: pool,
    platformCutCents: 0,
    members: rows.map(({ r: _r, ...m }) => m),
  };
}
function approvedEvidence(s, id, projectId) {
  const e = find(s.evidence, id);
  if (projectId && e.projectId !== projectId)
    fail('The evidence must belong to this project.');
  if (e.status !== 'reviewed') fail('Select independently reviewed evidence.');
  return e;
}
function settlementProject(s, settlementId) {
  const settlement = find(s.settlements, settlementId);
  const lot = find(s.lots, settlement.lotId);
  return find(s.projects, lot.projectId).id;
}
function paymentReference(s, value) {
  const reference = required(value, 300);
  if (
    s.allocations.some((allocation) =>
      [...(allocation.payments ?? []), ...(allocation.disbursements ?? [])]
        .some((receipt) => receipt.reference === reference),
    )
  ) fail('This payment reference is already recorded.');
  return reference;
}
function review(record, userId, decision, now, authorId = record.createdBy) {
  if (record.status !== 'submitted')
    fail('This record has already been reviewed.');
  if (authorId === userId)
    fail('Another steward must review this record.', 403);
  record.status = decision === 'approve' ? 'reviewed' : 'rejected';
  record.reviewedBy = userId;
  record.reviewedAt = now;
}
function safeLink(value) {
  if (!value) return '';
  let u;
  try {
    u = new URL(value);
  } catch {
    fail('Enter a complete HTTPS reference.');
  }
  if (u.protocol !== 'https:' || u.username || u.password)
    fail('Use an HTTPS reference without credentials.');
  return u.href;
}
export function applyCommand(
  previous,
  user,
  input,
  now = Date.now(),
  id = crypto.randomUUID(),
) {
  const s = structuredClone(previous),
    op = required(input.op, 50),
    p = input.payload ?? {};
  if (s.visibility === 'archived' && !isWorkspaceSafetyCommand(previous, user, input))
    fail('This co-op is archived. Safety changes and private operator reports remain available.');
  const contentIssue = communityContentIssue(op, p);
  if (contentIssue) fail(contentIssue);
  if (
    s.financialRecordsRedactedAt &&
    [
      'record_lot',
      'review_lot',
      'record_settlement',
      'review_settlement',
      'create_allocation',
      'approve_allocation',
      'record_payment',
      'review_payment',
      'record_disbursement',
      'review_disbursement',
      'record_retirement',
      'review_retirement',
    ].includes(op)
  )
    fail(
      'Financial recording is paused because account deletion removed supporting references. Existing anonymous totals remain available; contact support before recording further activity.',
      409,
    );
  if (op === 'request_membership' || op === 'accept_invitation') {
    if (op === 'accept_invitation') {
      const invitation = (s.invitations ?? []).find(
        (i) => i.tokenHash === p.tokenHash,
      );
      if (
        !invitation ||
        invitation.revoked ||
        invitation.expiresAt <= now ||
        invitation.usedBy
      )
        fail('This invitation is unavailable or expired.', 403);
      invitation.usedBy = user.id;
      invitation.usedAt = now;
    }
    if (op === 'request_membership' && s.visibility !== 'public')
      fail('This co-op is not accepting public membership requests.', 403);
    if (s.members.some((m) => m.userId === user.id))
      fail('You already have a membership or request.');
    if (s.members.length >= 500)
      fail('This pilot co-op has reached its member limit.');
    s.members.push({
      id,
      userId: user.id,
      name: required(p.name, 80),
      role: 'member',
      status: 'pending',
      joinedAt: now,
    });
  } else {
    const member = requireMember(s, user.id);
    switch (op) {
      case 'create_invitation': {
        requireSteward(s, user.id);
        s.invitations ??= [];
        if (s.invitations.length >= 200) fail('Invitation limit reached.');
        if (!/^[a-f0-9]{64}$/.test(p.tokenHash ?? ''))
          fail('Invalid invitation.');
        s.invitations.push({
          id,
          label: required(p.label, 120),
          tokenHash: p.tokenHash,
          expiresAt: now + 7 * 86400000,
          createdAt: now,
          revoked: false,
        });
        break;
      }
      case 'revoke_invitation':
        requireSteward(s, user.id);
        find(s.invitations ?? [], p.id).revoked = true;
        break;
      case 'update_organization':
        requireSteward(s, user.id);
        s.organization = {
          name: required(p.name, 160),
          kind: choose(p.kind, ['nonprofit', 'land_trust', 'community_group']),
          website: safeLink(required(p.website, 500)),
          region: required(p.region, 120),
          services: required(p.services, 1000),
          visibility: choose(p.visibility, ['members', 'public']),
          relationship: 'self_reported',
          updatedAt: now,
        };
        break;
      case 'record_partnership': {
        requireSteward(s, user.id);
        find(s.projects, p.projectId);
        approvedEvidence(s, p.evidenceId, p.projectId);
        s.partnerships ??= [];
        if (s.partnerships.length >= 100) fail('Partner record limit reached.');
        s.partnerships.push({
          id,
          projectId: p.projectId,
          name: required(p.name, 160),
          website: safeLink(required(p.website, 500)),
          role: required(p.role, 1000),
          agreementReference: required(p.agreementReference, 500),
          evidenceId: p.evidenceId,
          createdBy: user.id,
          createdAt: now,
          status: 'submitted',
        });
        break;
      }
      case 'invite_partner_representative':
      case 'respond_partner_invitation':
      case 'review_partner_representative':
      case 'end_partner_participation': {
        try { applyPartnerParticipation(s, user.id, op, p, now, id); }
        catch (error) { fail(error.message, error.status ?? 400); }
        break;
      }
      case 'review_partnership': {
        requireSteward(s, user.id);
        const partner = find(s.partnerships ?? [], p.id);
        approvedEvidence(s, partner.evidenceId, partner.projectId);
        review(
          partner,
          user.id,
          choose(p.decision, ['approve', 'reject']),
          now,
        );
        break;
      }
      case 'revoke_partnership': {
        requireSteward(s, user.id);
        const partner = find(s.partnerships ?? [], p.id);
        if (partner.status !== 'reviewed')
          fail('Only reviewed partner records can be revoked.');
        partner.revocationReason = required(p.reason, 1000);
        partner.status = 'revoked';
        partner.revokedBy = user.id;
        partner.revokedAt = now;
        break;
      }
      case 'record_assessment': {
        requireSteward(s, user.id);
        find(s.projects, p.projectId);
        const parcels = s.parcels.filter(
          (x) => x.projectId === p.projectId && x.status === 'reviewed',
        );
        if (!parcels.length)
          fail('Have another steward review at least one parcel first.');
        if (parcels.length !== pooledParcels(s, p.projectId).length)
          fail(
            'Review or withdraw every included parcel before assessing the pool.',
          );
        const geometry = landPoolGeometry(s, p.projectId);
        if (!geometry.complete)
          fail(
            'Review all boundaries, reconcile recorded areas, and resolve parcel overlaps before assessing the pool.',
          );
        s.assessments ??= [];
        s.assessments.push({
          id,
          projectId: p.projectId,
          program: required(p.program, 200),
          methodology: required(p.methodology, 200),
          source: safeLink(required(p.source, 500)),
          minimumSquareMetres: integer(p.minimumSquareMetres),
          parcelIds: parcels.map((x) => x.id),
          parcelSnapshot: parcelSnapshot(s, p.projectId),
          areaSquareMetres: geometry.areaSquareMetres,
          areaMethod: geometry.areaMethod,
          criteria: required(p.criteria, 4000),
          gaps: required(p.gaps, 4000),
          status: 'submitted',
          createdBy: user.id,
          createdAt: now,
        });
        break;
      }
      case 'review_assessment': {
        requireSteward(s, user.id);
        const assessment = find(s.assessments ?? [], p.id);
        if (p.decision === 'approve' && !assessmentIsCurrent(s, assessment))
          fail(
            'Land records changed or this legacy assessment lacks a snapshot. Record a new assessment before approval.',
          );
        review(
          assessment,
          user.id,
          choose(p.decision, ['approve', 'reject']),
          now,
        );
        break;
      }
      case 'update_regional_settings':
        requireSteward(s, user.id);
        if (s.settlements.length && p.currency !== (s.currency ?? 'USD'))
          fail(
            'Currency is locked after the first settlement to preserve existing amounts.',
          );
        s.currency = choose(p.currency, currencies);
        s.country = required(p.country, 120);
        break;
      case 'update_coop':
        requireSteward(s, user.id);
        if (s.operatorRestriction && p.visibility === 'public')
          fail('The operator has restricted public listing while a safety concern is reviewed. Contact support.', 403);
        s.name = required(p.name, 120);
        s.region = required(p.region, 120);
        s.summary = required(p.summary, 1000);
        s.visibility = choose(p.visibility, ['private', 'public']);
        break;
      case 'member_status': {
        requireSteward(s, user.id);
        const m = find(s.members, p.id);
        if (m.userId === s.ownerId)
          fail('The founding steward cannot be removed.');
        const status = choose(p.status, ['active', 'rejected', 'removed']);
        if (m.role === 'steward' && user.id !== s.ownerId)
          fail('Only the founding steward can remove another steward.', 403);
        m.status = status;
        break;
      }
      case 'member_role': {
        if (user.id !== s.ownerId)
          fail('Only the founding steward manages steward roles.', 403);
        const m = find(s.members, p.id);
        if (m.userId === s.ownerId)
          fail('The founding steward must remain a steward.');
        if (m.status !== 'active') fail('Activate this membership first.');
        m.role = choose(p.role, ['member', 'steward']);
        break;
      }
      case 'transfer_stewardship': {
        if (user.id !== s.ownerId)
          fail(
            'Only the founding steward can transfer this responsibility.',
            403,
          );
        const nextOwner = find(s.members, p.id);
        if (
          nextOwner.userId === user.id ||
          nextOwner.status !== 'active' ||
          nextOwner.role !== 'steward'
        )
          fail(
            'Choose another active steward who has agreed to take responsibility.',
          );
        if (p.confirmation !== 'TRANSFER')
          fail('Confirm the stewardship transfer.');
        s.ownerId = nextOwner.userId;
        break;
      }
      case 'block_member': {
        const target = find(s.members, p.id);
        if (target.userId === user.id) fail('You cannot block yourself.');
        s.blocks ??= [];
        if (
          !s.blocks.some(
            (b) => b.userId === user.id && b.blockedUserId === target.userId,
          )
        )
          s.blocks.push({
            userId: user.id,
            blockedUserId: target.userId,
            at: now,
          });
        break;
      }
      case 'unblock_member': {
        const target = find(s.members, p.id);
        s.blocks = (s.blocks ?? []).filter(
          (b) => b.userId !== user.id || b.blockedUserId !== target.userId,
        );
        break;
      }
      case 'leave':
        if (user.id === s.ownerId)
          fail(
            'The founding steward must archive the co-op rather than leave.',
          );
        member.status = 'removed';
        break;
      case 'archive':
        if (user.id !== s.ownerId)
          fail('Only the founding steward can archive.', 403);
        s.visibility = 'archived';
        break;
      case 'create_project':
        s.projects.push({
          id,
          name: required(p.name, 120),
          summary: required(p.summary, 2000),
          region: required(p.region, 120),
          kind: choose(p.kind, ['ecohedge', 'landscape', 'restoration', 'grassland']),
          status: 'proposed',
          visibility: 'members',
          createdBy: user.id,
          createdAt: now,
        });
        break;
      case 'project_status': {
        requireSteward(s, user.id);
        const project = find(s.projects, p.id);
        project.status = choose(p.status, [
          'proposed',
          'active',
          'completed',
          'cancelled',
        ]);
        project.visibility = choose(p.visibility, ['members', 'public']);
        break;
      }
      case 'save_boundary': {
        const parcel = find(s.parcels, p.parcelId);
        if (parcel.createdBy !== user.id) requireSteward(s, user.id);
        if (parcel.status === 'withdrawn')
          fail('This parcel was withdrawn from the pool.');
        let boundary;
        try {
          boundary = validateBoundary(p.geometry);
        } catch (e) {
          fail(e.message);
        }
        parcel.boundaries ??= [];
        if (parcel.boundaries.length >= 20)
          fail('Boundary version limit reached.');
        parcel.boundaries.push({
          id,
          ...boundary,
          consentReference: required(p.consentReference, 300),
          externalSearchAllowed: p.externalSearchAllowed === true,
          createdBy: user.id,
          createdAt: now,
          status: 'submitted',
        });
        break;
      }
      case 'review_boundary': {
        requireSteward(s, user.id);
        const parcel = find(s.parcels, p.parcelId);
        review(
          find(parcel.boundaries ?? [], p.id),
          user.id,
          choose(p.decision, ['approve', 'reject']),
          now,
        );
        break;
      }
      case 'revoke_satellite_consent': {
        const parcel = find(s.parcels, p.parcelId);
        if (parcel.createdBy !== user.id) requireSteward(s, user.id);
        const boundary = find(parcel.boundaries ?? [], p.id);
        boundary.externalSearchAllowed = false;
        boundary.consentRevokedAt = now;
        break;
      }
      case 'record_satellite_search': {
        const parcel = find(s.parcels, p.parcelId);
        if (parcel.createdBy !== user.id) requireSteward(s, user.id);
        const boundary = parcel.boundaries?.at(-1);
        if (
          !boundary ||
          boundary.id !== p.boundaryId ||
          boundary.status !== 'reviewed' ||
          !boundary.externalSearchAllowed
        )
          fail(
            'The current boundary must be reviewed and allow external searches.',
          );
        s.satelliteSearches ??= [];
        if (s.satelliteSearches.length >= 200)
          fail('Monitoring search limit reached.');
        s.satelliteSearches.push({
          id,
          ...p,
          createdBy: user.id,
          createdAt: now,
        });
        break;
      }
      case 'create_analysis_job': {
        const parcel = find(s.parcels, p.parcelId);
        if (parcel.createdBy !== user.id) requireSteward(s, user.id);
        const boundary = parcel.boundaries?.at(-1);
        if (!boundary || boundary.status !== 'reviewed')
          fail('Review the current boundary first.');
        const available = (s.satelliteSearches ?? [])
          .filter(
            (r) => r.parcelId === parcel.id && r.boundaryId === boundary.id,
          )
          .flatMap((r) => r.scenes);
        const before = find(available, p.beforeId),
          after = find(available, p.afterId);
        if (Date.parse(before.acquiredAt) >= Date.parse(after.acquiredAt))
          fail('Choose two different scenes in chronological order.');
        s.analysisJobs ??= [];
        if (s.analysisJobs.length >= 200) fail('Analysis job limit reached.');
        s.analysisJobs.push({
          id,
          parcelId: parcel.id,
          boundaryId: boundary.id,
          geometry: boundary.geometry,
          geometryCanonical: JSON.stringify(boundary.geometry),
          scenes: [before, after],
          algorithm: 'paired-ndvi-v1',
          createdBy: user.id,
          createdAt: now,
        });
        break;
      }
      case 'import_analysis': {
        const job = find(s.analysisJobs ?? [], p.jobId),
          parcel = find(s.parcels, job.parcelId);
        if (parcel.createdBy !== user.id) requireSteward(s, user.id);
        s.analysisResults ??= [];
        if (s.analysisResults.some((r) => r.jobId === job.id))
          fail('This job already has a receipt; create a new job for a rerun.');
        let receipt;
        try {
          receipt = validateAnalysis(p.receipt, job);
        } catch (e) {
          fail(e.message);
        }
        s.analysisResults.push({
          id,
          parcelId: parcel.id,
          jobId: job.id,
          receipt,
          status: 'submitted',
          createdBy: user.id,
          createdAt: now,
        });
        break;
      }
      case 'review_analysis': {
        requireSteward(s, user.id);
        const result = find(s.analysisResults ?? [], p.id);
        review(result, user.id, choose(p.decision, ['approve', 'reject']), now);
        result.reviewNote = required(p.note, 2000);
        break;
      }
      case 'record_observation': {
        const parcel = find(s.parcels, p.parcelId);
        if (parcel.createdBy !== user.id) requireSteward(s, user.id);
        const boundary = parcel.boundaries?.at(-1);
        if (!boundary || boundary.status !== 'reviewed')
          fail('Review the current boundary first.');
        const observedAt = integer(p.observedAt, 8640000000000000);
        if (observedAt > now)
          fail('Observation dates cannot be in the future.');
        let dateMetadata;
        try { dateMetadata = observationDateMetadata(p); }
        catch (error) { fail(error.message); }
        s.observations ??= [];
        if (s.observations.length >= 1000) fail('Observation limit reached.');
        s.observations.push({
          id,
          parcelId: parcel.id,
          boundaryId: boundary.id,
          observedAt,
          ...dateMetadata,
          method: required(p.method, 1000),
          finding: required(p.finding, 2000),
          reference: safeLink(p.reference),
          status: 'submitted',
          createdBy: user.id,
          createdAt: now,
        });
        break;
      }
      case 'review_observation':
        requireSteward(s, user.id);
        review(
          find(s.observations ?? [], p.id),
          user.id,
          choose(p.decision, ['approve', 'reject']),
          now,
        );
        break;
      case 'record_parcel': {
        find(s.projects, p.projectId);
        s.parcels.push({
          id,
          projectId: p.projectId,
          name: required(p.name, 120),
          landReference: required(p.landReference, 300),
          areaSquareMetres: integer(p.areaSquareMetres, 1e12),
          consentReference: required(p.consentReference, 300),
          notes: optional(p.notes, 2000),
          createdBy: user.id,
          createdAt: now,
          status: 'submitted',
        });
        break;
      }
      case 'review_parcel': {
        requireSteward(s, user.id);
        const parcel = find(s.parcels, p.id);
        review(
          parcel,
          user.id,
          choose(p.decision, ['approve', 'reject']),
          now,
          parcel.areaUpdatedBy ?? parcel.createdBy,
        );
        break;
      }
      case 'use_boundary_area': {
        const parcel = find(s.parcels, p.parcelId);
        if (parcel.createdBy !== user.id) requireSteward(s, user.id);
        if (parcel.status === 'withdrawn')
          fail('This parcel was withdrawn from the pool.');
        const boundary = parcel.boundaries?.at(-1);
        if (boundary?.status !== 'reviewed')
          fail('Review the current boundary first.');
        let measured;
        try {
          measured = validateBoundary(boundary.geometry).areaSquareMetres;
        } catch {
          fail('The current boundary is invalid.');
        }
        const area = Math.round(measured);
        if (area < 1)
          fail('The boundary is too small to record in whole square metres.');
        if (parcel.areaSquareMetres === area)
          fail('The recorded area already matches the rounded boundary area.');
        parcel.areaRevisions ??= [];
        parcel.areaRevisions.push({
          areaSquareMetres: parcel.areaSquareMetres,
          replacedBy: user.id,
          replacedAt: now,
        });
        parcel.areaSquareMetres = area;
        parcel.status = 'submitted';
        parcel.areaUpdatedBy = user.id;
        delete parcel.reviewedBy;
        delete parcel.reviewedAt;
        break;
      }
      case 'record_parcel_consent': {
        const parcel = find(s.parcels, p.parcelId);
        if (parcel.createdBy !== user.id) requireSteward(s, user.id);
        if (
          parcel.status !== 'reviewed' ||
          parcel.boundaries?.at(-1)?.status !== 'reviewed'
        )
          fail(
            'Review this parcel and its current boundary before recording pooling consent.',
          );
        if (p.attested !== true)
          fail(
            'Confirm that the referenced holder consent covers pooling this parcel and the current boundary.',
          );
        parcel.consents ??= [];
        if (parcel.consents.length >= 50)
          fail('Consent version limit reached.');
        parcel.consents.push({
          id,
          holder: required(p.holder, 200),
          authority: required(p.authority, 1000),
          reference: required(p.reference, 300),
          scope: required(p.scope, 2000),
          landSnapshot: consentLandSnapshot(parcel),
          attested: true,
          createdBy: user.id,
          createdAt: now,
          status: 'submitted',
        });
        break;
      }
      case 'review_parcel_consent': {
        requireSteward(s, user.id);
        const parcel = find(s.parcels, p.parcelId);
        const consent = find(parcel.consents ?? [], p.id);
        if (
          p.decision === 'approve' &&
          (consent !== parcel.consents.at(-1) ||
            parcel.status !== 'reviewed' ||
            parcel.boundaries?.at(-1)?.status !== 'reviewed' ||
            JSON.stringify(consent.landSnapshot) !==
              JSON.stringify(consentLandSnapshot(parcel)))
        )
          fail(
            'Land records changed. Record new pooling consent for the current parcel and boundary.',
          );
        consent.reviewNote = required(p.note, 2000);
        review(
          consent,
          user.id,
          choose(p.decision, ['approve', 'reject']),
          now,
        );
        break;
      }
      case 'revoke_parcel_consent': {
        const parcel = find(s.parcels, p.parcelId);
        if (parcel.createdBy !== user.id) requireSteward(s, user.id);
        const consent = find(parcel.consents ?? [], p.id);
        if (!['submitted', 'reviewed'].includes(consent.status))
          fail('This consent is no longer active.');
        consent.status = 'revoked';
        consent.revocationReason = required(p.reason, 1000);
        consent.revokedAt = now;
        consent.revokedBy = user.id;
        break;
      }
      case 'withdraw_parcel': {
        const parcel = find(s.parcels, p.parcelId);
        if (parcel.createdBy !== user.id) requireSteward(s, user.id);
        if (parcel.status === 'withdrawn')
          fail('This parcel was already withdrawn.');
        parcel.withdrawalReason = required(p.reason, 1000);
        parcel.status = 'withdrawn';
        parcel.withdrawnAt = now;
        parcel.withdrawnBy = user.id;
        break;
      }
      case 'record_authority': {
        requireSteward(s, user.id);
        if (s.authority?.status === 'reviewed')
          fail('Reviewed authority is frozen in this release.');
        approvedEvidence(s, p.evidenceId);
        s.authority = {
          id,
          legalName: required(p.legalName, 200),
          jurisdiction: required(p.jurisdiction, 160),
          reference: required(p.reference, 300),
          evidenceId: p.evidenceId,
          createdBy: user.id,
          createdAt: now,
          status: 'submitted',
        };
        break;
      }
      case 'review_authority': {
        requireSteward(s, user.id);
        if (!s.authority) fail('Submit an authority record first.');
        review(
          s.authority,
          user.id,
          choose(p.decision, ['approve', 'reject']),
          now,
        );
        break;
      }
      case 'create_event':
      case 'update_event': {
        const event = op === 'update_event' ? find(s.events ?? [], p.id) : null;
        if (event) {
          if (event.createdBy !== user.id) requireSteward(s, user.id);
          if (blockedInteraction(s, user.id, event.createdBy))
            fail('This event is unavailable for your account.', 403);
          if (event.status !== 'scheduled' || event.hidden || event.startsAt <= now)
            fail('Only upcoming, visible events can be edited.');
          if (p.projectId !== undefined && p.projectId !== event.projectId)
            fail('Keep an existing event with its original project.');
        }
        const projectId = event?.projectId ?? p.projectId;
        const project = find(s.projects, projectId);
        if (project.status === 'cancelled') fail('Choose an active project.');
        const startsAt = integer(p.startsAt, 8640000000000000),
          endsAt = integer(p.endsAt, 8640000000000000);
        if (
          startsAt <= now ||
          endsAt <= startsAt ||
          endsAt - startsAt > 7 * 86400000
        )
          fail('Choose a future start and an end within seven days.');
        const visibility = choose(p.visibility, ['members', 'public']);
        if (visibility === 'public') requireSteward(s, user.id);
        const timeZone = required(p.timeZone, 100);
        try {
          new Intl.DateTimeFormat('en', { timeZone });
        } catch {
          fail('Choose a valid time zone.');
        }
        s.events ??= [];
        if (!event && s.events.length >= 200)
          fail('This co-op has reached its event limit.');
        const capacity = integer(p.capacity, 500);
        const going = (event?.rsvps ?? []).filter(
          (r) => r.response === 'going' && membership(s, r.userId),
        ).length;
        if (capacity && capacity < going)
          fail('Capacity cannot be smaller than the current going responses.');
        const details = {
          title: required(p.title, 160),
          summary: required(p.summary, 2000),
          meetingDetails: required(p.meetingDetails, 2000),
          startsAt,
          endsAt,
          timeZone,
          visibility,
          capacity,
        };
        if (event) {
          Object.assign(event, details, {
            updatedAt: now,
            updatedBy: user.id,
            calendarSequence: (event.calendarSequence ?? 0) + 1,
          });
        } else {
          s.events.push({
            ...details,
            id,
            projectId,
            status: 'scheduled',
            hidden: false,
            rsvps: [],
            createdBy: user.id,
            createdAt: now,
            calendarSequence: 0,
          });
        }
        break;
      }
      case 'cancel_event': {
        const event = find(s.events ?? [], p.id);
        if (event.createdBy !== user.id) requireSteward(s, user.id);
        if (event.status === 'cancelled') fail('This event is already cancelled.');
        event.status = 'cancelled';
        event.cancelReason = required(p.reason, 500);
        event.cancelledBy = user.id;
        event.cancelledAt = now;
        event.calendarSequence = (event.calendarSequence ?? 0) + 1;
        break;
      }
      case 'event_rsvp': {
        const event = find(s.events ?? [], p.id);
        if (blockedInteraction(s, user.id, event.createdBy))
          fail('This event is unavailable for your account.', 403);
        if (
          event.status !== 'scheduled' ||
          event.hidden ||
          event.endsAt <= now ||
          find(s.projects, event.projectId).status === 'cancelled'
        )
          fail('This event is no longer accepting responses.');
        const response = choose(p.response, [
          'going',
          'interested',
          'not_going',
        ]);
        const going = event.rsvps.filter(
          (x) =>
            x.userId !== user.id &&
            x.response === 'going' &&
            membership(s, x.userId),
        ).length;
        if (response === 'going' && event.capacity && going >= event.capacity)
          fail(
            'All available places are taken. Choose interested or check back later.',
          );
        event.rsvps = event.rsvps.filter((x) => x.userId !== user.id);
        event.rsvps.push({ userId: user.id, response, at: now });
        break;
      }
      case 'post_comment': {
        const update = find(s.updates, p.updateId);
        if (blockedInteraction(s, user.id, update.createdBy))
          fail('This discussion is unavailable for your account.', 403);
        if (update.hidden) fail('This discussion is unavailable.');
        s.comments ??= [];
        if (s.comments.length >= 1000)
          fail('This co-op has reached its discussion limit.');
        s.comments.push({
          id,
          updateId: update.id,
          text: required(p.text, 2000),
          author: member.name,
          createdBy: user.id,
          createdAt: now,
          hidden: false,
        });
        break;
      }
      case 'remove_comment': {
        const comment = find(s.comments ?? [], p.id);
        if (comment.createdBy !== user.id) requireSteward(s, user.id);
        comment.hidden = true;
        break;
      }
      case 'report_content': {
        const kind = choose(p.kind, ['update', 'comment', 'event']);
        const target = find(
          kind === 'update'
            ? s.updates
            : kind === 'comment'
              ? (s.comments ?? [])
              : (s.events ?? []),
          p.targetId,
        );
        if (target.hidden) fail('This content is already hidden.');
        s.reports ??= [];
        if (s.reports.length >= 500)
          fail('This co-op has reached its report limit. Send a private operator report at /report/ for help.');
        if (
          s.reports.some(
            (x) =>
              x.createdBy === user.id &&
              x.targetId === target.id &&
              x.kind === kind &&
              x.status === 'open',
          )
        )
          fail('Your report is already awaiting review.');
        s.reports.push({
          id,
          kind,
          targetId: target.id,
          reason: required(p.reason, 2000),
          status: 'open',
          createdBy: user.id,
          createdAt: now,
        });
        break;
      }
      case 'resolve_report': {
        requireSteward(s, user.id);
        const report = find(s.reports ?? [], p.id);
        if (report.status !== 'open') fail('This report was already resolved.');
        const decision = choose(p.decision, ['hide', 'dismiss']);
        const target = find(
          report.kind === 'update'
            ? s.updates
            : report.kind === 'comment'
              ? (s.comments ?? [])
              : (s.events ?? []),
          report.targetId,
        );
        if (target.createdBy === user.id)
          fail('Another steward must review a report about your content.', 403);
        if (decision === 'hide') target.hidden = true;
        report.status = decision === 'hide' ? 'hidden' : 'dismissed';
        report.note = required(p.note, 1000);
        report.reviewedAt = now;
        report.reviewedBy = user.id;
        break;
      }
      case 'post_update': {
        find(s.projects, p.projectId);
        const visibility = choose(p.visibility, ['members', 'public']);
        if (visibility === 'public') requireSteward(s, user.id);
        s.updates.push({
          id,
          projectId: p.projectId,
          text: required(p.text, 2000),
          visibility,
          author: member.name,
          createdBy: user.id,
          createdAt: now,
          hidden: false,
        });
        break;
      }
      case 'hide_update': {
        requireSteward(s, user.id);
        find(s.updates, p.id).hidden = true;
        break;
      }
      case 'create_task': {
        find(s.projects, p.projectId);
        s.tasks.push({
          id,
          projectId: p.projectId,
          title: required(p.title, 200),
          due: optional(p.due, 10),
          status: 'open',
          assignee: null,
          createdBy: user.id,
          createdAt: now,
        });
        break;
      }
      case 'task_status': {
        const t = find(s.tasks, p.id);
        const status = choose(p.status, ['claimed', 'completed', 'open']);
        if (t.assignee && t.assignee !== user.id && !isSteward(s, user.id))
          fail('This action is assigned to another member.', 403);
        t.status = status;
        t.assignee = status === 'open' ? null : user.id;
        t.completedAt = status === 'completed' ? now : null;
        break;
      }
      case 'submit_agreement': {
        find(s.projects, p.projectId);
        const parcelIds = p.parcelIds ?? [];
        const coverage = agreementCoverageSnapshot(s, p.projectId, parcelIds);
        if (!coverage)
          fail(
            'Select the specific parcels this agreement covers within this project.',
          );
        for (const parcelId of parcelIds) {
          const parcel = find(s.parcels, parcelId);
          if (parcel.createdBy !== user.id) requireSteward(s, user.id);
          if (!parcelConsentIsCurrent(parcel))
            fail(
              'Record independently reviewed pooling consent for every covered parcel first.',
            );
        }
        s.agreements.push({
          id,
          projectId: p.projectId,
          kind: choose(p.kind, ['enrollment', 'easement', 'carbon_rights']),
          parcelIds: [...parcelIds].sort((a, b) => a.localeCompare(b)),
          parcelSnapshot: coverage,
          jurisdiction: required(p.jurisdiction, 120),
          holder: required(p.holder, 160),
          notes: required(p.notes, 3000),
          reference: safeLink(p.reference),
          createdBy: user.id,
          createdAt: now,
          status: 'submitted',
          reviewNote: '',
          recordingReference: '',
          executionReference: '',
        });
        break;
      }
      case 'review_agreement': {
        requireSteward(s, user.id);
        const a = find(s.agreements, p.id);
        if (['execution_recorded', 'revoked'].includes(a.status))
          fail(
            'Executed references are frozen; submit a new agreement to record amendments.',
          );
        if (a.createdBy === user.id)
          fail('Another steward must review the agreement.', 403);
        const status = choose(p.status, [
          'changes_requested',
          'reviewed',
          'execution_recorded',
        ]);
        if (status !== 'changes_requested' && !agreementIsCurrent(s, a))
          fail(
            'This agreement lacks current parcel coverage or pooling consent. Submit a new agreement for the current parcels.',
          );
        if (status === 'execution_recorded' && a.status !== 'reviewed')
          fail('Review the agreement before recording execution.');
        a.reviewNote = required(p.note, 2000);
        if (status === 'execution_recorded') {
          a.executionReference = required(p.executionReference, 300);
          if (a.kind === 'easement')
            a.recordingReference = required(p.recordingReference, 300);
        }
        a.status = status;
        a.reviewedBy = user.id;
        a.reviewedAt = now;
        break;
      }
      case 'revoke_agreement': {
        requireSteward(s, user.id);
        const agreement = find(s.agreements, p.id);
        if (agreement.status !== 'execution_recorded')
          fail('Only an executed agreement record can be revoked.');
        agreement.revocationReason = required(p.reason, 1000);
        agreement.status = 'revoked';
        agreement.revokedBy = user.id;
        agreement.revokedAt = now;
        break;
      }
      case 'submit_evidence': {
        find(s.projects, p.projectId);
        const asset = p.asset ?? null;
        if (asset && asset.uploaderId !== user.id)
          fail('You may only attach your own upload.', 403);
        s.evidence.push({
          id,
          projectId: p.projectId,
          title: required(p.title, 160),
          method: required(p.method, 200),
          period: required(p.period, 80),
          reference: safeLink(p.reference),
          notes: required(p.notes, 2000),
          asset,
          createdBy: user.id,
          createdAt: now,
          status: 'submitted',
        });
        if (!asset && !s.evidence.at(-1).reference)
          fail('Attach a file or an HTTPS evidence reference.');
        break;
      }
      case 'review_evidence': {
        requireSteward(s, user.id);
        const e = find(s.evidence, p.id);
        review(e, user.id, choose(p.decision, ['approve', 'reject']), now);
        e.reviewNote = required(p.note, 1500);
        break;
      }
      case 'propose_charter': {
        requireSteward(s, user.id);
        const electorate = s.members
          .filter((m) => m.status === 'active')
          .map((m) => m.id);
        if (
          !Array.isArray(p.shares) ||
          p.shares.length !== electorate.length ||
          p.shares.some((m) => !electorate.includes(m.id))
        )
          fail('Provide one share for every active member.');
        const shares = p.shares.map((m) => ({
          id: m.id,
          name: find(s.members, m.id).name,
          shareBps: integer(m.shareBps, 10000),
        }));
        allocateCents(0, shares, p.stewardshipBps, p.treasuryBps);
        const days = integer(p.days, 30);
        if (days < 1)
          fail(
            'Voting must stay open for at least one day unless everyone votes.',
          );
        s.proposals.push({
          id,
          title: required(p.title, 160),
          text: required(p.text, 4000),
          shares,
          stewardshipBps: p.stewardshipBps,
          treasuryBps: p.treasuryBps,
          electorate,
          quorum: Math.ceil((electorate.length * 2) / 3),
          closesAt: now + days * 86400000,
          status: 'open',
          votes: [],
          createdBy: user.id,
          createdAt: now,
        });
        break;
      }
      case 'vote': {
        const proposal = find(s.proposals, p.id);
        if (proposal.status !== 'open' || now >= proposal.closesAt)
          fail('Voting is closed.');
        if (!proposal.electorate.includes(member.id))
          fail('You are not in this proposal’s frozen electorate.', 403);
        const choice = choose(p.choice, ['approve', 'oppose', 'abstain']);
        proposal.votes = proposal.votes.filter((v) => v.memberId !== member.id);
        proposal.votes.push({ memberId: member.id, choice, at: now });
        break;
      }
      case 'close_proposal': {
        requireSteward(s, user.id);
        const proposal = find(s.proposals, p.id);
        if (proposal.status !== 'open')
          fail('This proposal is already closed.');
        if (
          now < proposal.closesAt &&
          proposal.votes.length < proposal.electorate.length
        )
          fail('Wait for the deadline or all eligible members to vote.');
        const passed =
          proposal.votes.length >= proposal.quorum &&
          proposal.votes.filter((v) => v.choice === 'approve').length >
            proposal.electorate.length / 2;
        proposal.status = passed ? 'adopted' : 'not_adopted';
        proposal.closedAt = now;
        if (passed)
          s.charters.push({
            id: proposal.id,
            version: s.charters.length + 1,
            shares: proposal.shares,
            stewardshipBps: proposal.stewardshipBps,
            treasuryBps: proposal.treasuryBps,
            adoptedAt: now,
          });
        break;
      }
      case 'record_lot': {
        requireSteward(s, user.id);
        find(s.projects, p.projectId);
        if (s.authority?.status !== 'reviewed')
          fail('Review the co-op legal authority record first.');
        const parcels = pooledParcels(s, p.projectId);
        if (!parcels.length || !parcels.every(parcelConsentIsCurrent))
          fail(
            'Review the parcel rights and current pooling consent record for every included parcel first.',
          );
        if (!landPoolGeometry(s, p.projectId).complete)
          fail(
            'Resolve missing boundaries, area differences, and parcel overlaps first.',
          );
        if (
          !parcels.every((parcel) =>
            parcelAgreementCovered(s, p.projectId, parcel.id, [
              'enrollment',
              'easement',
            ]),
          )
        )
          fail(
            'Record an executed land participation agreement covering every current parcel first.',
          );
        if (
          !parcels.every((parcel) =>
            parcelAgreementCovered(s, p.projectId, parcel.id, [
              'carbon_rights',
            ]),
          )
        )
          fail(
            'Record executed carbon-rights authority covering every current parcel first.',
          );
        approvedEvidence(s, p.evidenceId, p.projectId);
        const start = integer(p.serialStart),
          end = integer(p.serialEnd);
        if (end < start || end - start + 1 > 1e9) fail('Invalid serial range.');
        const registry = required(p.registry, 100).toLowerCase(),
          program = required(p.program, 100).toLowerCase(),
          serialPrefix = required(p.serialPrefix, 200),
          vintage = required(p.vintage, 20);
        if (
          s.lots.some(
            (l) =>
              l.registry === registry &&
              l.program === program &&
              l.serialPrefix === serialPrefix &&
              Math.max(start, l.serialStart) <= Math.min(end, l.serialEnd),
          )
        )
          fail('This serial range overlaps an existing holding.');
        s.lots.push({
          id,
          projectId: p.projectId,
          registry,
          program,
          serialPrefix,
          vintage,
          method: required(p.method, 160),
          serialStart: start,
          serialEnd: end,
          units: end - start + 1,
          unit: 'tCO2e',
          evidenceId: p.evidenceId,
          reference: required(p.reference, 300),
          status: 'submitted',
          createdBy: user.id,
          createdAt: now,
        });
        break;
      }
      case 'review_lot': {
        requireSteward(s, user.id);
        review(
          find(s.lots, p.id),
          user.id,
          choose(p.decision, ['approve', 'reject']),
          now,
        );
        break;
      }
      case 'record_settlement': {
        requireSteward(s, user.id);
        const lot = find(s.lots, p.lotId);
        if (lot.status !== 'reviewed')
          fail('Review the issuance record first.');
        approvedEvidence(s, p.evidenceId, lot.projectId);
        const units = integer(p.units, 1e9),
          cents = integer(p.cents);
        if (!units || !cents) fail('Record positive units and settled cash.');
        const used = s.settlements
          .filter((x) => x.lotId === lot.id && x.status !== 'rejected')
          .reduce((sum, x) => sum + x.units, 0);
        if (used + units > lot.units)
          fail('The holding does not have enough uncommitted units.');
        const reference = required(p.reference, 300);
        if (s.settlements.some((x) => x.reference === reference))
          fail('This settlement reference is already recorded.');
        s.settlements.push({
          id,
          lotId: lot.id,
          units,
          cents,
          currency: s.currency ?? 'USD',
          reference,
          evidenceId: p.evidenceId,
          status: 'submitted',
          createdBy: user.id,
          createdAt: now,
        });
        break;
      }
      case 'review_settlement': {
        requireSteward(s, user.id);
        review(
          find(s.settlements, p.id),
          user.id,
          choose(p.decision, ['approve', 'reject']),
          now,
        );
        break;
      }
      case 'create_allocation': {
        requireSteward(s, user.id);
        const settlement = find(s.settlements, p.settlementId);
        if (settlement.status !== 'reviewed')
          fail('Review the settlement receipt first.');
        if (s.allocations.some((a) => a.settlementId === settlement.id))
          fail('This receipt already has an allocation.');
        const charter = find(s.charters, p.charterId);
        s.allocations.push({
          id,
          settlementId: settlement.id,
          charterId: charter.id,
          amounts: allocateCents(
            settlement.cents,
            charter.shares,
            charter.stewardshipBps,
            charter.treasuryBps,
          ),
          status: 'draft',
          createdBy: user.id,
          createdAt: now,
          payments: [],
          disbursements: [],
        });
        break;
      }
      case 'approve_allocation': {
        requireSteward(s, user.id);
        const a = find(s.allocations, p.id);
        if (a.createdBy === user.id)
          fail('Another steward must approve the allocation.', 403);
        if (a.status !== 'draft') fail('Allocation is already approved.');
        a.status = 'approved';
        a.approvedBy = user.id;
        break;
      }
      case 'record_payment': {
        requireSteward(s, user.id);
        const a = find(s.allocations, p.id);
        if (a.status !== 'approved') fail('Approve the allocation first.');
        const m = find(a.amounts.members, p.memberId);
        if (!m.cents) fail('No payment is due for this member.');
        if (
          a.payments.some((x) => x.memberId === m.id && x.status !== 'rejected')
        )
          fail('A payment record already exists for this member.');
        approvedEvidence(s, p.evidenceId, settlementProject(s, a.settlementId));
        const reference = paymentReference(s, p.reference);
        a.payments.push({
          id,
          memberId: m.id,
          cents: m.cents,
          reference,
          evidenceId: p.evidenceId,
          status: 'submitted',
          createdBy: user.id,
          createdAt: now,
        });
        break;
      }
      case 'review_payment': {
        requireSteward(s, user.id);
        const a = find(s.allocations, p.id);
        review(
          find(a.payments, p.paymentId),
          user.id,
          choose(p.decision, ['approve', 'reject']),
          now,
        );
        break;
      }
      case 'record_disbursement': {
        requireSteward(s, user.id);
        const allocation = find(s.allocations, p.id);
        if (allocation.status !== 'approved') fail('Approve the allocation first.');
        const budget = choose(p.budget, ['stewardship', 'treasury']);
        const cents = integer(p.cents);
        if (!cents) fail('Record a positive completed external payment amount.');
        const available = allocationReconciliation(allocation)[budget]
          .remainingUnrecordedCents;
        if (cents > available)
          fail('This payment exceeds the unrecorded amount in this allocation budget.');
        approvedEvidence(s, p.evidenceId, settlementProject(s, allocation.settlementId));
        const reference = paymentReference(s, p.reference);
        allocation.disbursements ??= [];
        allocation.disbursements.push({
          id,
          budget,
          cents,
          recipientLabel: required(p.recipientLabel, 200),
          purpose: required(p.purpose, 1000),
          reference,
          evidenceId: p.evidenceId,
          status: 'submitted',
          createdBy: user.id,
          createdAt: now,
        });
        break;
      }
      case 'review_disbursement': {
        requireSteward(s, user.id);
        const allocation = find(s.allocations, p.id);
        review(
          find(allocation.disbursements ?? [], p.disbursementId),
          user.id,
          choose(p.decision, ['approve', 'reject']),
          now,
        );
        break;
      }
      case 'record_retirement': {
        requireSteward(s, user.id);
        const settlement = find(s.settlements, p.settlementId);
        if (settlement.status !== 'reviewed')
          fail('Select a reviewed settlement.');
        approvedEvidence(s, p.evidenceId, settlementProject(s, settlement.id));
        const units = integer(p.units, 1e9);
        if (
          !units ||
          units +
            s.retirements
              .filter(
                (x) =>
                  x.settlementId === settlement.id && x.status !== 'rejected',
              )
              .reduce((n, x) => n + x.units, 0) >
            settlement.units
        )
          fail('Retirement exceeds the recorded transferred units.');
        s.retirements.push({
          id,
          settlementId: settlement.id,
          units,
          beneficiary: required(p.beneficiary, 200),
          reference: required(p.reference, 300),
          evidenceId: p.evidenceId,
          status: 'submitted',
          createdBy: user.id,
          createdAt: now,
        });
        break;
      }
      case 'review_retirement': {
        requireSteward(s, user.id);
        review(
          find(s.retirements, p.id),
          user.id,
          choose(p.decision, ['approve', 'reject']),
          now,
        );
        break;
      }
      default:
        fail('Unknown operation.');
    }
  }
  s.updatedAt = now;
  s.audit.push({
    id,
    sequence: s.audit.length + 1,
    action: op,
    actorId: user.id,
    at: now,
  });
  const capacityIssue = workspaceCapacityIssue(previous, s, user, input, { pendingReceipt: true });
  if (capacityIssue) fail(capacityIssue, 409);
  return s;
}
export function publicWorkspace(s) {
  const profileNeedsReview = !!communityContentIssue('update_coop', s);
  const projects = s.projects
    .filter(
      (p) =>
        p.visibility === 'public' &&
        p.status !== 'cancelled' &&
        !communityContentIssue('create_project', p),
    )
    .map(({ id, name, summary, region, kind, status }) => ({
      id,
      name,
      summary,
      region,
      kind,
      status,
    }));
  return {
    id: s.id,
    name: profileNeedsReview ? 'Community co-op' : s.name,
    region: profileNeedsReview ? '' : s.region,
    country: profileNeedsReview ? '' : (s.country ?? ''),
    summary: profileNeedsReview
      ? 'This co-op profile is awaiting moderation.'
      : s.summary,
    projects,
    events: (s.events ?? [])
      .filter(
        (e) =>
          e.visibility === 'public' &&
          !e.hidden &&
          !communityContentIssue('create_event', e) &&
          projects.some((p) => p.id === e.projectId),
      )
      .map(
        ({
          id,
          projectId,
          title,
          summary,
          startsAt,
          endsAt,
          timeZone,
          status,
        }) => ({
          id,
          projectId,
          title,
          summary,
          startsAt,
          endsAt,
          timeZone,
          status,
        }),
      ),
    organization:
      s.organization?.visibility === 'public' &&
      !communityContentIssue('update_organization', s.organization)
        ? s.organization
        : null,
    updates: s.updates
      .filter(
        (u) =>
          u.visibility === 'public' &&
          !u.hidden &&
          !communityContentIssue('post_update', u) &&
          projects.some((p) => p.id === u.projectId),
      )
      .map(({ id, projectId, text, createdAt }) => ({
        id,
        projectId,
        text,
        createdAt,
      })),
    memberCount: s.members.filter((m) => m.status === 'active').length,
  };
}
export function memberView(s, userId) {
  const viewer = requireMember(s, userId);
  const steward = isSteward(s, userId);
  const capacity = workspaceCapacity(s);
  const copy = structuredClone(s);
  const reviewAvailable = steward && s.visibility !== 'archived' && !capacity.growthPaused;
  const canReview = (record, status = 'submitted', financial = false) =>
    !!(reviewAvailable && (!financial || !s.financialRecordsRedactedAt) &&
      record.status === status && record.createdBy && record.createdBy !== userId);
  for (const records of [copy.observations ?? [], copy.analysisResults ?? []])
    for (const record of records) record.canReview = canReview(record);
  for (const records of [copy.lots, copy.settlements, copy.retirements])
    for (const record of records) record.canReview = canReview(record, 'submitted', true);
  for (const allocation of copy.allocations) {
    allocation.canReview = canReview(allocation, 'draft', true);
    for (const receipt of [...(allocation.payments ?? []), ...(allocation.disbursements ?? [])])
      receipt.canReview = canReview(receipt, 'submitted', true);
  }
  copy.members = copy.members
    .filter((m) => m.status === 'active' || steward || m.userId === userId)
    .map(({ userId: uid, ...m }) => ({ ...m, isYou: uid === userId }));
  copy.ownerId = undefined;
  copy.blocks = (s.blocks ?? [])
    .filter((b) => b.userId === userId)
    .map((b) => {
      const target = s.members.find((m) => m.userId === b.blockedUserId);
      return { memberId: target?.id, name: target?.name ?? 'Former member' };
    });
  copy.invitations = steward
    ? (copy.invitations ?? []).map(
        ({ tokenHash: _tokenHash, usedBy, ...i }) => ({
          ...i,
          used: !!usedBy,
        }),
      )
    : [];
  copy.partnerships = (s.partnerships ?? []).map(p => partnerParticipationView(s, p, userId)).filter(Boolean);
  copy.assessments = steward ? (copy.assessments ?? []) : [];
  copy.parcels = copy.parcels.filter((p) => steward || p.createdBy === userId);
  copy.satelliteSearches = (copy.satelliteSearches ?? []).filter((x) =>
    copy.parcels.some((p) => p.id === x.parcelId),
  );
  copy.analysisJobs = (copy.analysisJobs ?? []).filter((x) =>
    copy.parcels.some((p) => p.id === x.parcelId),
  );
  copy.analysisResults = (copy.analysisResults ?? []).filter((x) =>
    copy.parcels.some((p) => p.id === x.parcelId),
  );
  copy.observations = (copy.observations ?? []).filter((x) =>
    copy.parcels.some((p) => p.id === x.parcelId),
  );
  copy.agreements = copy.agreements.filter(
    (a) => steward || a.createdBy === userId,
  );
  copy.evidence = copy.evidence.filter(
    (e) => steward || e.createdBy === userId,
  );
  copy.evidence.forEach((e) => {
    if (e.asset) {
      delete e.asset.objectKey;
      delete e.asset.uploaderId;
    }
  });
  // Stewards retain reported material for moderation; their personal feed is
  // filtered by the client. Other members never receive blocked social content.
  const visibleSocial = (record) =>
    steward || !blockedInteraction(s, userId, record.createdBy);
  copy.updates = copy.updates
    .filter((u) => (!u.hidden || steward) && visibleSocial(u))
    .map((u) => ({
      ...u,
      authorMemberId: s.members.find((m) => m.userId === u.createdBy)?.id,
      blocked: blockedInteraction(s, userId, u.createdBy),
    }));
  copy.events = (copy.events ?? [])
    .filter((e) => (!e.hidden || steward) && visibleSocial(e))
    .map(({ createdBy, cancelledBy: _cancelledBy, updatedBy: _updatedBy, rsvps, ...e }) => ({
      ...e,
      authorMemberId: s.members.find((m) => m.userId === createdBy)?.id,
      blocked: blockedInteraction(s, userId, createdBy),
      isOrganizer: createdBy === userId,
      goingCount: rsvps.filter(
        (x) => x.response === 'going' && membership(s, x.userId),
      ).length,
      yourResponse: rsvps.find((x) => x.userId === userId)?.response ?? '',
      attendees:
        steward || createdBy === userId
          ? rsvps
              .filter((x) => membership(s, x.userId))
              .map((x) => ({
                name: membership(s, x.userId).name,
                response: x.response,
              }))
          : [],
    }));
  copy.comments = (copy.comments ?? [])
    .filter(
      (c) =>
        (!c.hidden || steward) &&
        visibleSocial(c) &&
        copy.updates.some((u) => u.id === c.updateId),
    )
    .map(({ createdBy, ...c }) => ({
      ...c,
      authorMemberId: s.members.find((m) => m.userId === createdBy)?.id,
      isYou: createdBy === userId,
      blocked: blockedInteraction(s, userId, createdBy),
    }));
  copy.reports = (copy.reports ?? [])
    .filter((r) => steward || r.createdBy === userId)
    .map(({ createdBy, reviewedBy: _reviewedBy, ...r }) => ({
      ...r,
      isYou: createdBy === userId,
    }));
  copy.audit = copy.audit.map(
    ({
      actorId,
      requestHash: _requestHash,
      stateHash: _stateHash,
      commitmentNonce: _commitmentNonce,
      ...a
    }) => ({
      ...a,
      actor:
        s.members.find((m) => m.userId === actorId)?.name ?? 'Former member',
    }),
  );
  return {
    state: copy,
    capacity,
    role: viewer.role,
    memberId: viewer.id,
    isOwner: s.ownerId === userId,
  };
}
