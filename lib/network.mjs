...
export function publicWorkspace(s) {
  ...
  return {
    ...
    summary: s.summary,
    projects,
    events: (s.events ?? []).filter(e => e.visibility === 'public' && !e.hidden && projects.some(p => p.id === e.projectId))
      .map(({id, projectId, title, summary, startsAt, endsAt, timeZone, status}) => ({id, projectId, title, summary, startsAt, endsAt, timeZone, status})),
    organization:
