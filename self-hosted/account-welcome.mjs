...
export function welcomePage({
  ...
  const emailEnabled = socialProviders.some((p) => p.id === 'email');
  ...
  const intro = recovery
    ...
    : emailEnabled
      ? 'Sign in or create your account to conserve with your community.'
      : register
  ...
  const emailForm =
    emailEnabled && !recovery
      ? `<form class="credentials" method="post" action="/auth/social/start"><input type="hidden" name="provider" value="email"><input type="hidden" name="action" value="login">${returnField}${field('email', 'Email address', 'email', 'autocomplete="email" maxlength="254"')}<p class="hint">We’ll email you a one-time sign-in link. Open it in this browser within 10 minutes.</p><button class="primary">Continue with email</button></form><p class="link-note">New here? The same step creates your account. No password to remember.</p>`
      : '';
  ...
  const authChoices = recovery
    ? credentialForm
    : emailEnabled
      ? `${emailForm}${providerChoices}<details class="password-option"><summary>Use a VergeCommon username and password</summary>${credentialForm}</details>`
      : `${credentialForm}${providerChoices}`;
