...
export function accountPage({
  ...
  if (!user) {
    ...
    body += `<h2>Change password</h2><p>Changing your password signs out other sessions and revokes all device tokens.</p>${form('password', pass('currentPassword', 'Current password') + pass('newPassword', 'New password', 'new-password'), 'Change password')}<h2>Your records</h2><p><a href="/account/export">Download account and co-op records available to you</a></p><section id="close-account" class="account-closure"><h2>Permanently close your login</h2><p>This removes your login and revokes access. It does not delete shared co-op records, which remain with their co-op for accountability. Transfer founder responsibility to another active steward from Members and arrange any record-removal requests first. You cannot undo login closure. <a href="/privacy/#your-choices">Read about account closure and retained records</a>.</p>${form('close', pass() + input('confirmation', 'Type CLOSE to confirm'), 'Permanently close login')}</section>${form('logout', '', 'Sign out')}`;
  }
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Account · VergeCommon</title><style>body{font:17px/1.6 system-ui;margin:0;background:#f6f8f1;color:#173a2c}main{max-width:620px;padding:30px 22px;margin:auto}a{color:#145937}h1{font-size:2.6rem}h2{margin-top:2rem}form{display:grid;gap:14px;margin:16px 0}label{display:grid;gap:6px}input,select,button{font:inherit;padding:11px;border:1px solid #8b9e90;border-radius:7px}button,.button{background:#17513b;color:white;cursor:pointer}section{border:2px solid #17513b;padding:18px;background:#e6f1df}code{overflow-wrap:anywhere}li{border-bottom:1px solid #b3c3b4;padding:8px}nav{display:flex;gap:20px}.notice{padding:15px;background:#faefcc}details{margin:20px 0}summary{cursor:pointer;font-weight:650;padding:12px 0;min-height:44px}.field-hint{font-size:14px;color:#4a6354;font-weight:400}.account-actions{display:flex;align-items:center;flex-wrap:wrap;gap:16px;margin:24px 0}.button{display:inline-flex;padding:12px 18px;border-radius:7px;text-decoration:none;min-height:44px}.account-devices{border-top:1px solid #b3c3b4;border-bottom:1px solid #b3c3b4;padding:6px 0}input,select{min-width:0;width:100%;box-sizing:border-box}a:focus-visible,button:focus-visible,input:focus-visible,select:focus-visible,summary:focus-visible{outline:3px solid #ad6810;outline-offset:3px}section h2{margin-top:0}.account-closure{margin:28px 0;border-color:#b3c3b4;background:transparent;scroll-margin-top:24px}</style></head><body><main><nav><a href="/">VergeCommon</a><a href="/network/">Community</a></nav>${message ? `<p role="status" class="notice">${escape(message)}</p>` : ''}${body}<footer><p><a href="/privacy/">Privacy</a> · <a href="/support/">Get help</a> · <a href="https://github.com/jdhart81/verge-common/tree/build/coop-launch-readiness">Source</a></p></footer></main></body></html>`;
}
