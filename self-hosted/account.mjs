...
export function accountPage({
  ...
  if (!user) {
    ...
    body += `<h2>Change password</h2><p>Changing your password signs out other sessions and revokes all device tokens.</p>${form('password', pass('currentPassword', 'Current password') + pass('newPassword', 'New password', 'new-password'), 'Change password')}<h2>Your records</h2><p><a href="/account/export">Download account and co-op records available to you</a></p><section id="close-account" class="account-closure"><h2>Permanently delete your account</h2><p>This revokes all access and removes your authored personal content, land records and uploaded evidence. Shared governance and numeric records may remain with identity fields removed; other members’ own content is separate. Transfer founder responsibility to another active steward from Members if your co-op has other active members. You cannot undo deletion. <a href="/privacy/#your-choices">Read about deletion and retained records</a>.</p>${form('close', pass() + input('confirmation', 'Type DELETE to confirm'), 'Permanently delete account')}</section>${form('logout', '', 'Sign out')}`;
  }
