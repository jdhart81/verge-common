...
Google OAuth type: Web application. JavaScript origin: `https://vergecommon.com`. Scopes: `openid`, email and profile only. The owner explicitly approved `hartjustin6@gmail.com` as the public support contact and approved Google's API Services User Data Policy. The console now confirms "OAuth configuration created!" and shows the saved VergeCommon name/contact with Testing status. After owner approval, `VergeCommon Supabase Web` was created with the origin and Supabase callback above. Client ID: `509382893613-8ea0lnprjjgo3j7thtdpvo018a93ehj3.apps.googleusercontent.com`. Its downloaded credential JSON was verified for this project and restricted to owner-only permissions; it is outside the repository. The Supabase Google form has its public client ID prepared; owner secret entry and Save remain pending. The new Google client ID/secret were securely installed in the dedicated server's existing root-only configuration, preserving other values and all enable flags; no running container was restarted. Homepage `https://vergecommon.com/`, privacy link `https://vergecommon.com/privacy/` and authorized domain `vergecommon.com` were saved and the console confirmed branding changes. Audience remains Testing; real sign-in acceptance is incomplete.
