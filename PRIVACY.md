...
Removed members lose private access. Their earlier records remain with the co-op unless their account is deleted. Archiving stops ordinary growth and public discovery but preserves authorized reads and bounded safety actions such as reports, membership removal and withdrawal of consent. Account deletion is available on the self-hosted website and native app; it is distinct from leaving, removing a member, or archiving a co-op.
...
Partner representative invitations share the reviewed partnership’s organization name, website and agreed role with one selected active member. Invitation, response, role, authority reference and review details are visible only to that representative and active stewards. Supporting authority evidence keeps the existing submitter/steward access rules; the invitation does not reveal private agreement files. This workflow grants no account privileges and does not verify institutional or legal authority. Account deletion removes attributable declarations and withdraws reviews whose reviewer or supporting evidence was erased.
...
The native beta source supports deliberate selection of photos, PDF and text evidence. Images are resized to at most 2,048 pixels and reencoded as JPEG without source metadata; PDF/text contents and metadata are unchanged. Before sending, the app shows the selected co-op/project and asks the person to describe the evidence. No camera or location permission is requested. Prepared bytes, stable retry IDs, destination and submission receipts are saved in complete-protection device storage excluded from device backup. Entries are bound to the account ID returned by the service, not to a display name. There are no automatic background sends. Successful submission removes prepared bytes only after a durable local receipt is saved. Signing out/account closure does not delete this independent local queue; its own confirmed removal control remains available. Removing a local copy does not withdraw already received evidence. Journal drafts and independently exported copies have separate lifecycles.

Prepared native discussion replies also use protected account-bound storage excluded from backups and journal exports. The app saves the exact reply text and request identity before sending, retains uncertain attempts for explicit same-account retry, and does not send automatically. A confirmed successful local receipt removes the stored text. Definitively rejected first attempts can be replaced; an earlier uncertain attempt retains its original text and identity. Signing out/account closure does not remove this separate queue. Local removal controls do not withdraw online replies.

Idempotent uploads retain a private hashed attempt key. Active receipts associate it with a byte/metadata fingerprint and asset; deleted or expired attempts retain only the hashed key, status and creation time to prevent reuse. The server does not preserve file contents or filenames in those terminal receipts.
