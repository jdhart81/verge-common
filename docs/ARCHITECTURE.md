...
The shared website and hosted MCP are deployed; [feature acceptance](FEATURE_ACCEPTANCE.md) records the live synthetic account checks and links to the deployment/recovery receipt. Physical-device acceptance, Apple signing and TestFlight/App Store uploads remain separate release checks. Do not describe a source or simulator build as a distributed mobile release.
