...
Updated 19 September 2026 for the [launch build candidate](LAUNCH_BUILD_2026-09-19.md), extending the earlier `5aa1a5f` review packet. The final candidate commit and deployment receipt remain to be recorded by the release coordinator. This is not a completed App Store Connect declaration, upload or approval. Native release evidence and remaining gates belong in [APPLE_RELEASE.md](APPLE_RELEASE.md).
...
>
> Search public co-ops, load more communities, and discover their projects and events. Create an account in the app to see your member workspaces, share updates, and submit selected field observations for steward review. Keep notes offline, export a journal backup, and choose when to share a draft.
>
...
Local-only records are distinct: journal drafts, operator lookup receipts and the hidden-co-op display list stay in protected app storage until a chosen submission, export or status request. Operator receipts contain no report text and persist independently across account switches; removal loses lookup access without withdrawing the operator's received report. They may be included in device backups according to device settings. Journal exports exclude the Keychain credential. Account deletion preserves the local journal; **Journal tools → Delete all local drafts** is a separate confirmed action. Exported copies and other people's independent content are not under the app's automatic control.
...
1. Resolve the intended seller/team, proposed bundle ID and signing setup. Current local tests and unsigned simulator compilation pass; the earlier license error is historical. Build and validate an archive from a recorded commit, then record version/build and archive checksum. Preserve the current CI evidence separately from device results.
...
8. Verify final policy/support links, operator-report availability, optional mail fallback, privacy report, declarations and private review credentials. Upload only the signed qualified candidate; record Apple's processing and review status. Use TestFlight for the beta without charging for access.
