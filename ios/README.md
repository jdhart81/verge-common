...
Drafts are held in app storage and may be included in device backups according to device settings. Exports leave that protected storage and follow the user's chosen destination. This version requests no location, camera, tracking or notification access and has no analytics SDK. Selected observations and member posts leave the device only after explicit submission. Private workspace responses are kept in memory, with no URL cache or cookie store. There is no automatic journal upload or synchronization. This is not a complete App Store privacy disclosure: inventory the final website/auth services and native build before submission.
