...
await test('saved plans round-trip with actions and notes', () => {
  const plan = addTask(
...
await test('invitation excludes private notes and completed actions', () => {
...
await test('invalid and oversized saved data are handled deliberately', () => {
...
await test('adding an action does not mutate the previous plan', () => {
